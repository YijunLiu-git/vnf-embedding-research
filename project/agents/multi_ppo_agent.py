        ...
        for epoch in range(self.ppo_epochs):
            ...
            for start_idx in range(0, batch_size, self.mini_batch_size):
                ...
                if all(isinstance(s, Data) for s in batch_states):
                    ...
                    states_tensor = torch.stack([torch.tensor(s) if not isinstance(s, torch.Tensor) else s 
                                               for s in batch_states]).to(self.device)
                    batch_state_embeddings = self.gnn_encoder(states_tensor) if hasattr(self, 'gnn_encoder') else states_tensor
...
def test_ppo_agent():
    ...
    agent = MultiPPOAgent("test_ppo", state_dim=8, action_dim=10, edge_dim=4, config=config)
    ...
    # 测试经验存储
    for i in range(40):  # 超过rollout_length以触发学习
